        ...
        <div class="card_table">
            <div>
                <div class="card_table1">
                    <br><br>
                    <?php if ($data['hak_akses'] == 'admin_prod') { ?>
                        <a href="<?= BASEURL ?>/tambah_SKP" class="tambah"><i class="fas fa-user-plus"></i><span> Tambah</span></a>
                    <?php } ?>
                </div>
                </div>
                <div class="card_table2">
                    ...
                    <?php if($data['keluar_produk']['halaman_saat_ini'] > 1): ?>
                        <a href = "<?= BASEURL ?>/lihat_SKP/<?= $data['keluar_produk']['halaman_saat_ini'] - 1; ?>"  class = 'pageLebih'>&laquo;</a>
                    <?php endif; ?>

                    <?php for($p = 1; $p <= $data['keluar_produk']['jumlah_halaman']; $p++): ?>
                        <?php if($p == $data['keluar_produk']['halaman_saat_ini']): ?>
                            <a href= "<?= BASEURL ?>/lihat_SKP/<?= $p; ?>" class = 'page_aktif'><?= $p; ?></a>
                        <?php else: ?>
                            <a href= "<?= BASEURL ?>/lihat_SKP/<?= $p; ?>"  class = 'page'><?= $p; ?></a>
                        <?php endif; ?>
                    <?php endfor; ?>
                    
                    <?php if($data['keluar_produk']['halaman_saat_ini'] < $data['keluar_produk']['jumlah_halaman']): ?>
                        <a href = "<?= BASEURL ?>/lihat_SKP/<?= $data['keluar_produk']['halaman_saat_ini'] + 1; ?>"  class = 'pageLebih'>&raquo;</a>
                    <?php endif; ?>
